...
export default function Page() {
  const slides = [
    ...
    {
      src: "https://source.unsplash.com/1600x900/?data,analytics,charts",
      title: "Clear insights",
      desc: "See where money goes with real-time charts.",
    },
    {
    ...
    <section className="relative min-h-screen">
      ...
      <div className="relative grid min-h-screen grid-cols-1 lg:grid-cols-12">
        ...
        <aside className="relative order-last hidden lg:order-first lg:col-span-6 lg:block">
          {/* Give the panel a solid height so the viewport can fill it */}
          <div className="relative h-screen overflow-hidden bg-gray-900">
            <Carousel
              // Force the Embla viewport (the div with `overflow-hidden`) to h-full
                ...
                {slides.map((s, i) => (
                  <CarouselItem key={i} className="h-full">
                    <div className="relative h-full">
                      {/* Image fills the slide; keep it absolute inside a relative h-full wrapper */}
                      <img
                        src={s.src}
                        alt={s.title}
        ...
        <main className="order-first flex items-center justify-center px-6 py-10 lg:order-last lg:col-span-6 lg:px-16">
          <div className="w-full max-w-md">
            ...
            <p className="mt-6 text-center text-sm text-gray-400">
              Trouble signing in? <a href="/" className="underline transform transition-all ease-out hover:shadow-lg text-indigo-600">Return home</a>
            </p>
          </div>
        </main>
      </div>
